...
<html>
  <head>
    <title> Car Pooling - Gestisci prenotazioni </title>
  ...
  <body>
    ...
    <?php
      ...
      $stm = $dbh->prepare("SELECT pa.*, v.*, p.* FROM Prenotazione p INNER JOIN Autista a ON p.idAutista = a.idAutista INNER JOIN Viaggio v ON p.idViaggio = v.idViaggio INNER JOIN Passeggero pa ON p.idPasseggero = pa.idPasseggero WHERE p.idAutista=:id AND p.stato='Attesa'");
